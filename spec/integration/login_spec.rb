...
describe 'Login' do
  path '/api/v1/login' do
    ...
    post 'Sends verification code' do
      ...
      response '200', 'Verification code was sent' do
        ...
        context 'When user and his pair already existed' do
          ...
          before do
            expect_any_instance_of(Login::DownloadAndCreatePair).not_to receive(:call!)
          end
        ...
        context 'When user and his pair didn\'t exist' do
          ...
          before do
            expect_any_instance_of(Login::FetchPairData).to receive(:call).and_return(fake_pair_data)
          end
          ...
          it 'downloads pair data and creates records', save: :data do |example|
            ...
            user1 = User.joins(:pair).find_by(email: 'john@example.org', pairs: {pair_nr: 2})
            expect(user1).not_to be_nil

            user2 = User.joins(:pair).find_by(email: 'email2@example.com', pairs: {pair_nr: 2})
      ...
      response '503', 'Error with connection to external api occured, verification code wasn\'t sent' do
        let(:data) { {email: 'john@example.org', pair_nr: 2} }
    ...
    patch 'Returns authentication token' do
      ...
      response '200', 'Verification code and emails are correct, token is returned' do
        let(:data) { {email: 'john@example.org', verification_code: '123456'} }
      ...
      response '401', 'Verification code and emails are incorrect, error is returned' do
        let(:data) { {email: 'john@example.org', verification_code: '123456'} }
